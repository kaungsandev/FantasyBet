<div class="grid bg-white rounded-md shadow-md dark:bg-gray-700 shadow-sky-400">
    {{-- header --}}
    <div class="flex flex-col w-full px-4 py-4 space-y-2 border-b">
        <div class="flex flex-row items-center justify-between text-xl font-bold dark:text-gray-400">
            <p class=" dark:text-gray-200">{{ $player['web_name'] }}</p>
            <p><span class="pr-1">&pound;</span>{{ number_format($player['now_cost'] / 10, 1) }}</p>
        </div>

        <div class="flex flex-row items-center justify-between text-xs font-extrabold text-gray-500 dark:text-gray-400">
            <p>{{ $team->name }}</p>
    ...
    {{-- Stats --}}
    <div class="grid self-stretch w-full grid-rows-4 space-y-1 bg-gray-200 shadow-inner dark:bg-gray-400 rounded-b-md">
        <div class="grid w-full grid-cols-4 gap-4 px-4 py-2 text-xs text-center text-gray-400 dark:text-gray-200">
            <p>Form</p>
            <p>Points</p>
            <p>Total</p>
            <p>TSB</p>
        ...
        <div
            class="grid grid-cols-4 gap-4 px-4 py-2 text-xs font-bold text-center text-gray-600 place-items-center dark:text-gray-100">
            <p>{{ $player['form'] }}</p>
            <p>{{ $player['event_points'] }} pts</p>
            <p>{{ $player['total_points'] }} pts</p>
            <p>{{ $player['selected_by_percent'] }}%</p>
        </div>
        <div class="grid grid-cols-4 gap-4 px-4 py-2 text-xs text-center text-gray-400 dark:text-gray-200">
            <p>Goal Scored</p>
            <p>Goal Conceded</p>
            <p>Assists</p>
            <p>Clean Sheets</p>
