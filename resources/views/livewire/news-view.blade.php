 ...
 <a href="{{$new->url}}" target="_blank">
  <div class="bg-white rounded-lg w-full h-full">
    ...
    @if ($new->urlToImage)
    <img src="{{$new->urlToImage}}" alt="People" class="w-full object-cover rounded-b-lg h-100">
    @else
    <img src="{{asset('img/default_news_cover.jpg')}}" alt="People" class="w-full rounded-b-lg object-cover h-64 sm:h-48 md:h-64">
    @endif
  </div>
</a>
@endforeach
